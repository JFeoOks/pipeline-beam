 ...
 *
 * <p>NcJdbcIO source returns a bounded collection of {@code T} as a {@code PCollection<T>}. T is the
 ...
 * <p>For example:
 * <pre>{@code
 * pipeline.apply(NcJdbcIO.<KV<Integer, String>>read()
 *   .withDataSourceConfiguration(NcJdbcIO.DataSourceConfiguration.create(
 *          "com.mysql.jdbc.Driver", "jdbc:mysql://hostname:3306/mydb")
 *        .withUsername("username")
 *        .withPassword("password"))
 *   .withQuery("select id,name from Person")
 *   .withCoder(KvCoder.of(BigEndianIntegerCoder.of(), StringUtf8Coder.of()))
 *   .withRowMapper(new NcJdbcIO.RowMapper<KV<Integer, String>>() {
 ...
 *   .withCoder(KvCoder.of(BigEndianIntegerCoder.of(), StringUtf8Coder.of()))
 *   .withStatementPreparator(new NcJdbcIO.StatementPreparator() {
 ...
 * pipeline
 *   .apply(...)
 *   .apply(NcJdbcIO.<KV<Integer, String>>write()
 *      .withDataSourceConfiguration(NcJdbcIO.DataSourceConfiguration.create(
 *            "com.mysql.jdbc.Driver", "jdbc:mysql://hostname:3306/mydb")
 *          .withUsername("username")
 *          .withPassword("password"))
 *      .withStatement("insert into Person values(?, ?)")
 *      .withPreparedStatementSetter(new NcJdbcIO.PreparedStatementSetter<KV<Integer, String>>() {
 ...
 * }</pre>
 *
 * <p>NB: in case of transient failures, Beam runners may execute parts of NcJdbcIO.Write multiple
...
public class NcJdbcIO {

    private static final Logger LOG = LoggerFactory.getLogger(NcJdbcIO.class);
    ...
    /**
     * An interface used by {@link NcJdbcIO.Read} for converting each row of the {@link ResultSet} into
    ...
    /**
     * An interface used by the NcJdbcIO Write to set the parameters of the {@link PreparedStatement}
    ...
    public abstract static class ReadAll<ParameterT, OutputT>
        ...
        public ReadAll<ParameterT, OutputT> withDataSourceConfiguration(
            ...
            checkArgument(configuration != null, "NcJdbcIO.readAll().withDataSourceConfiguration"
                    + "(configuration) called with null configuration");
            return toBuilder().setDataSourceConfiguration(configuration).build();
        }

        public ReadAll<ParameterT, OutputT> withQuery(String query) {
            checkArgument(query != null, "NcJdbcIO.readAll().withQuery(query) called with null query");
            return withQuery(ValueProvider.StaticValueProvider.of(query));
        }
        ...
        public ReadAll<ParameterT, OutputT> withParameterSetter(
            ...
            checkArgument(parameterSetter != null,
                    "NcJdbcIO.readAll().withParameterSetter(parameterSetter) called "
        ...
        public ReadAll<ParameterT, OutputT> withRowMapper(RowMapper<OutputT> rowMapper) {
            checkArgument(rowMapper != null,
                    "NcJdbcIO.readAll().withRowMapper(rowMapper) called with null rowMapper");
            return toBuilder().setRowMapper(rowMapper).build();
        }

        public ReadAll<ParameterT, OutputT> withCoder(Coder<OutputT> coder) {
            checkArgument(coder != null, "NcJdbcIO.readAll().withCoder(coder) called with null coder");
            return toBuilder().setCoder(coder).build();
        }
...
//                }
    ...
    private static class Reparallelize<T> extends PTransform<PCollection<T>, PCollection<T>> {
        ...
        public PCollection<T> expand(PCollection<T> input) {
            ...
            // The current approach is necessary only to support the particular case of NcJdbcIO where
